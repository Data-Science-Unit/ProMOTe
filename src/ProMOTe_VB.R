VB_gaussian_update <- function(d, t, rho, tau, iota, hyperparameters, initial_Cstar, initial_Dstar, initial_pstar, initial_qstar, initial_rstar, N, M, K, epsilon, sex = NULL, birth_conds = NULL, male_conds = NULL, female_conds = NULL, cond_list) {
  ...
  on.exit(return(list(posterior.parameters = list(theta_star = theta_star, a_star = a_star, b_star = b_star, u_star = u_star, v_star = v_star, alpha_star = alpha_star, beta_star = beta_star, C_star = C_star, p_star = p_star, q_star = q_star, r_star = r_star, D_star = D_star), n_steps = n_steps, final_step_size = param_difference, elbo = elbos[1:n_steps], cond_list = cond_list)))
  ...
  while(param_difference > epsilon) {
    ...
    beta_star <- beta_hyper + Edt_sqEz/2#(t(d*(t**2))%*%expected_z)/2
    beta_star <- beta_star - (EdtEz**2)/(2*EdEz)#((tdEz)**2)/(2*(dEz))
    ...
    #Update D_star
    D_star_old <- D_star
    # D_star <- sqrt(pi) * exp(((q_star**2)/(4*r_star)) + p_star)
    # D_star <- D_star * (1 - erf((-q_star+2*r_star*tau)/(2*sqrt(r_star))))
    # D_star <- D_star/(2*sqrt(r_star))
    ...
    elbo_likelihood <- sum(expected_z%*%expected_log_gamma) + sum(t(expected_d%*%expected_log_pi)%*%expected_z) + sum(t((1-expected_d) %*% expected_log_pi_complement)%*%expected_z) -0.5*sum(EdEz)*log(2*pi) -0.5*sum(t(expected_d%*%expected_log_sigma_squared) %*% expected_z) -0.5*sum(t(expected_d%*%expected_mu2_sigma2)%*%expected_z) + sum(t(expected_dt%*%expected_mu_sigma2)%*%expected_z) - 0.5*sum(t(expected_dt2%*%expected_sigma2_inverse)%*%expected_z)
